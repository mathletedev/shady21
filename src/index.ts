...
let state: number[] = new Array(SIZE).fill(0);
...
let enabled = prompt("Enable AI moves? (y/n)")?.toLowerCase().startsWith("y");

const select = (i: number, p: number) => {
	if (i === SIZE - 1) i = SIZE - 2;
	if (turn !== p || !valid(i)) return;
			...
			`${
				enabled
					? turn === 1
						? "Player"
						: "Computer"
					: `${turn === 1 ? "Blue" : "Red"} player`
			} won!`
		);

	turn *= -1;
	if (enabled && turn === -1) ai();
};

const valid = (i: number) => state[i] === 0 && state[i + 1] === 0;

const full = () => {
	for (let i = 0; i < SIZE - 1; i++) if (valid(i)) return false;
	return true;
};

const hover = (i: number, o: boolean) => {
	if (i === SIZE - 1) i = SIZE - 2;
	if (!valid(i)) return;
...
const set = (i: number, c: string) =>
	((board.children[i] as HTMLDivElement).style.backgroundColor = c);
